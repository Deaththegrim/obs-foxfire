...
int main(void)
{
	...
	for (int i = 0; i < 4; i++) ff_analysis_push(a, buf, FF_HOP, &f); /* 5 hops of silence total */
	CHECK(f.peaks[12] > 0.f && f.peaks[12] < p0 && f.peaks[12] >= f.bands[12]);
	for (int i = 0; i < 96; i++) ff_analysis_push(a, buf, FF_HOP, &f); /* 101 hops of silence total */
	CHECK(f.bands[2] < 0.01f);
	for (int i = 0; i < 99; i++) ff_analysis_push(a, buf, FF_HOP, &f); /* 200 hops of silence total */
	...
	memset(buf, 0, sizeof buf);
	for (int i = 0; i < 3; i++) ff_analysis_push(a, buf, FF_HOP, &f);
	CHECK(f.beat < b0 * 0.9f);
	CHECK(f.beat > 0.f);
	CHECK(f.beat_count == before);
	for (int i = 0; i < 57; i++) ff_analysis_push(a, buf, FF_HOP, &f); /* 60 silent hops total since b0 */
	CHECK(f.beat == 0.f);
	...
	ff_analysis_destroy(a); a = ff_analysis_create(48000); ff_analysis_set_params(a, &p);
	{
		...
		for (size_t i = 0; i < 96; i++) stream[i] = 0.9f;
		for (size_t i = 0; i < 96; i++) stream[2048 + i] = 0.9f;
	...
	ff_analysis_destroy(a); a = ff_analysis_create(48000); ff_analysis_set_params(a, &p);
	{
		...
		for (int hop = 0; hop < hops; hop++) {
			for (size_t i = 0; i < FF_HOP; i++, s++) {
				int hot = 0;
				for (int c = 0; c < 5; c++) if (s >= starts[c] && s < starts[c] + 96) { hot = 1; break; }
				buf[i] = hot ? 0.9f : 0.f;
			}
			ff_analysis_push(a, buf, FF_HOP, &f);
		}
	...
	/* band calibration: a 0 dBFS 1 kHz sine lights its band near 1.0, a -60 dBFS one near 0 */
	{
		...
		ff_analysis_destroy(a); a = ff_analysis_create(48000); ff_analysis_set_params(a, &p);
		ph = 0; for (int i = 0; i < 40; i++) { tone(buf, FF_HOP, 1000.f, sr, 1.0f, &ph); ff_analysis_push(a, buf, FF_HOP, &f); }
		float mx = 0.f; for (int b = b1k - 1; b <= b1k + 1; b++) if (b >= 0 && b < FF_BANDS && f.bands[b] > mx) mx = f.bands[b];
		CHECK(mx >= 0.95f);

		ff_analysis_destroy(a); a = ff_analysis_create(48000); ff_analysis_set_params(a, &p);
		ph = 0; for (int i = 0; i < 40; i++) { tone(buf, FF_HOP, 1000.f, sr, 0.001f, &ph); ff_analysis_push(a, buf, FF_HOP, &f); }
		mx = 0.f; for (int b = b1k - 1; b <= b1k + 1; b++) if (b >= 0 && b < FF_BANDS && f.bands[b] > mx) mx = f.bands[b];
		CHECK(mx <= 0.05f);
	}
