...
static void source_cb(void *param, obs_source_t *src, const struct audio_data *data, bool muted)
{
	...
	if (muted) {
		...
		while (left) { uint32_t n = left > FF_HOP ? FF_HOP : left; feed(a, z, NULL, n); left -= n; }
		return;
	}
	feed(a, (const float *)data->data[0], data->data[1] ? (const float *)data->data[1] : NULL, data->frames);
}
...
{
	if (!a->weak) return false;
	if (!obs_weak_source_expired(a->weak)) return true;
	obs_weak_source_release(a->weak); a->weak = NULL;
	return false;
}
...
static void on_source_signal(void *param, calldata_t *cd)
{
	...
	if (a->mode != FF_AUDIO_SOURCE || have_live_source(a)) { pthread_mutex_unlock(&a->conn_lock); return; }
	connect_source(a);
...
void ff_audio_configure(struct ff_audio *a, enum ff_audio_mode mode, const char *source_name, const struct ff_analysis_params *p)
{
	...
	if (!(same && (a->master_connected || have_live_source(a)))) {
		...
		if (mode == FF_AUDIO_MASTER) {
			...
			if (!connect_source(a))
				obs_log(LOG_WARNING, "audio: source '%s' not found or has no audio; rendering silence until it appears", a->source_name);
		}
	}
	pthread_mutex_unlock(&a->conn_lock);
}
