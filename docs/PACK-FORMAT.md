...
### Wipes driven by a PNG

The highest-value transition a pack can ship is a **luma wipe**: a greyscale image that decides
the *order* in which pixels change, with a soft band travelling through it from dark to light.
`basics/effects/wipe.effect` is OBS's own algorithm (`luma_wipe_transition.effect`), reproduced so
that every mask anyone already owns behaves the way they expect:

```hlsl
float sf = max(softness, 0.001);
float mixv = clamp((t * (1.0 + sf) - luma) / sf, 0.0, 1.0);
return lerp(a, b, mixv);
```

The mask is an ordinary pack texture with `bool user = true`, so the panel offers a file picker
and a streamer can drop in their own art without authoring a pack at all. Two rules for the art:

* **It must span the full 0–255.** A mask whose darkest pixel is 20 leaves the frame untouched for
  the first fifth of the cut and then jumps; one whose brightest is 235 finishes early and holds a
  frozen frame. Neither shows up in a still.
* **Decide whether its shape matters.** `fit = 0` stretches the mask to the canvas, which is right
  for anything that reads across the frame; `fit = 1` covers instead, keeping the art's proportions
  so a round iris stays round on a 16:9 canvas rather than becoming an ellipse.

The starter set in `basics` is generated by `tools/make-wipe-masks.py` in the packs repo — geometry
that has to be exact (a true angular sweep, equal star points, a centred iris) is computed rather
than drawn. Genuinely drawn art is just as valid; it simply has to obey the two rules above.

### Covering sweeps, and why they need a hold

A wipe *blends*: both scenes are on screen together. A stinger *covers*: for a stretch of frames
nothing but the art is visible, the cut happens underneath, and the viewer never sees the two
scenes meet. `basics/effects/sweep.effect` does the second, from two images — a `band` whose
**alpha** is the leading edge, and a `fill` for what the covered frame shows, tinted through a
gradient so one preset serves any palette.

The part that is easy to get wrong, and was wrong here first: **coverage has to last**. The
obvious construction — the leading edge finishes its travel exactly at the cut, the trailing edge
starts there — is completely covered at *one instant* and leaks a sliver of scene a hair either
side. Measured: two milliseconds off the swap and the outgoing scene shows at full strength; with
the hold removed, 10% of a recorded frame was scene A. At 30 fps a 300 ms cut is nine frames and
none of them will land on that instant.

So a covering transition needs a `hold`: a window, centred on the cut, in which both edges are
parked off-frame and the whole canvas is art.

```hlsl
float half_hold = hold * 0.5;
float close_by  = max(swap_point - half_hold, 0.001);   /* edge finishes travelling here */
float open_at   = min(swap_point + half_hold, 0.999);   /* the other one starts here */
```
